...
import { FavoriteButtonRef, FavoriteButtonProps } from './types'
import styles from './favorite_button.module.scss'

const FavoriteButtonBase = forwardRef<FavoriteButtonRef, FavoriteButtonProps>((props, ref) => {
	const { className, type = 'button', active = false, ...rest } = props

	const colorHeart = active ? '#FF0000' : '#999'

	return (
		<button className={multipleStyles([styles.container, active && styles['container-active'], className])} {...rest} ref={ref}>
			<svg viewBox="0 0 18 16" fill="none">
				<path d="M9 5C9 2.79086 7.20914 1 5 1C2.79086 1 1 2.79086 1 5C1 9 7 13 9 15" stroke={colorHeart} strokeLinecap="round" />
				<path d="M9.27686 11C10.3119 10.1153 11.4675 9.09688 12.4677 8" stroke={colorHeart} />
				<path
...
export type FavoriteButton = FavoriteButtonRef

export const FavoriteButton = observer(FavoriteButtonBase)
